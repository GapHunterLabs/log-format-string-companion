...
dependencies {
    ...
    intellijPlatform {
        ...
        // same reasoning already documented in sql-concatenation-companion's
        // build.gradle.kts: this plugin's whole detection surface (logging
        // calls in Java/Kotlin/Python source) is done as plain-text/regex
        // analysis over the open editor's Document, same "hand-rolled over
        // plain text" principle already proven catalog-wide. This also
        // means the inspection runs on ANY file type (.java/.kt/.py)
        // without needing a per-language PSI grammar dependency -- see
        // LocalInspectionTool registered without a `language` filter in
        // plugin.xml.
